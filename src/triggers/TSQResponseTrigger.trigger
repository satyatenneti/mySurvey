trigger TSQResponseTrigger on Trifecta_Survey_Question_Response__c (before insert) {
    ...
    Map<Id,Trifecta_Survey_Question__c> questionsMap = new Map<Id,Trifecta_Survey_Question__c>([select Id, Type__c, (select Id, Value__c, Weightage__c from Trifecta_Survey_Question_Choices__r) from Trifecta_Survey_Question__c where id in :questionIds and Type__c ='Multi Choice' ]);
    for (Trifecta_Survey_Question_Response__c eachAnswer : Trigger.new) {
